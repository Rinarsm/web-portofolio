(function () {
  const translations = {
    en: {
      ...
      'viewer.back': '← Back to Certificates',
      'viewer.eyebrow': 'Credential Viewer',
      'viewer.description': 'View the complete credential directly on this page. The PDF is rendered inside the portfolio for a more consistent experience on mobile devices.',
      'viewer.loading': 'Loading credential…',
      'viewer.error': 'The credential could not be displayed in the built-in viewer.',
      'viewer.download': '↓ Download PDF',
      'viewer.openOriginal': 'Open Original PDF',
      'viewer.invalid': 'Credential not found.',
    ...
    id: {
      ...
      'viewer.back': '← Kembali ke Sertifikat',
      'viewer.eyebrow': 'Viewer Kredensial',
      'viewer.description': 'Lihat kredensial lengkap langsung di halaman ini. PDF ditampilkan di dalam portofolio agar lebih konsisten pada perangkat mobile.',
      'viewer.loading': 'Memuat kredensial…',
      'viewer.error': 'Kredensial tidak dapat ditampilkan melalui viewer bawaan.',
      'viewer.download': '↓ Unduh PDF',
      'viewer.openOriginal': 'Buka PDF Asli',
      'viewer.invalid': 'Kredensial tidak ditemukan.',
  ...
  const credentialFiles = {
    'its-data-analytics-rina-rismawati-2025.pdf': 'cert.item1Title',
    'coding-camp-2025-machine-learning-engineer-rina-rismawati.pdf': 'cert.item2Title',
    'dicoding-JMZVLNWGQXN9.pdf': 'cert.item3Title',
    'dicoding-L4PQ2RKMOZO1.pdf': 'cert.item4Title',
    'dicoding-1RXYE5RRKZVM.pdf': 'cert.item5Title',
    'dicoding-N9ZO9R5O6XG5.pdf': 'cert.item6Title',
    'dicoding-GRX5JNRDYX0M.pdf': 'cert.item7Title',
    'dicoding-98XWEQJEWXM3.pdf': 'cert.item8Title',
    'dicoding-JLX194YE5P72.pdf': 'cert.item9Title',
    'dicoding-0LZ0RY6Q3P65.pdf': 'cert.item10Title',
    'dicoding-6RPNRY1DQX2M.pdf': 'cert.item11Title',
    'dicoding-1RXYEL099ZVM.pdf': 'cert.item12Title',
    'dicoding-MRZMN8483PYQ.pdf': 'cert.item13Title',
    'dicoding-KEXL783NWXG2.pdf': 'cert.item14Title',
    'dicoding-KEXLY8NY4ZG2.pdf': 'cert.item15Title',
    'dicoding-0LZ06087RZ65.pdf': 'cert.item16Title',
    'dicoding-6RPNY1M4RZ2M.pdf': 'cert.item17Title',
    'dicoding-QLZ92RE07X5D.pdf': 'cert.item18Title',
    'dicoding-JLX13NQ85P72.pdf': 'cert.item19Title'
  };

  function getCredentialFile() {
    const viewer = document.getElementById('credential-viewer');
    if (!viewer) return null;
    const requested = new URLSearchParams(window.location.search).get('file') || '';
    return Object.prototype.hasOwnProperty.call(credentialFiles, requested) ? requested : null;
  }

  function updateCredentialViewerTitle(language) {
    const titleElement = document.getElementById('credential-title');
    if (!titleElement) return;
    const file = getCredentialFile();
    if (!file) {
      titleElement.textContent = translations[language]['viewer.invalid'];
      return;
    }
    const key = credentialFiles[file];
    titleElement.textContent = translations[language][key] || translations[language]['viewer.eyebrow'];
  }
  ...
  function setLanguage(lang) {
    ...
    updateCredentialViewerTitle(language);
    safeSet('portfolio-language', language);
    setTabState({ language });

    // Reveal the page only after every translatable element and the language
    // switcher have been updated, preventing an English flash between pages.
  ...
  // Built-in credential PDF viewer (Credential Viewer page only).
  const credentialViewer = document.getElementById('credential-viewer');

  if (credentialViewer) {
    const file = getCredentialFile();
    const pagesContainer = document.getElementById('pdf-pages');
    const statusElement = document.getElementById('pdf-status');
    const downloadLink = document.getElementById('credential-download');
    const originalLink = document.getElementById('credential-open-original');
    ...
    function showViewerError() {
      if (statusElement) {
        ...
        statusElement.dataset.i18n = 'viewer.error';
        statusElement.textContent = translations[currentLanguage()]['viewer.error'];
      }
      if (fallbackElement) fallbackElement.hidden = false;
    }

    async function renderPdfPages() {
      if (!loadedPdf || !pagesContainer) return;
      const version = ++renderVersion;
      pagesContainer.innerHTML = '';

      const availableWidth = Math.max(280, Math.min(pagesContainer.clientWidth || 900, 1080));
    ...
    async function initializeCredentialViewer() {
      if (!file) {
        if (statusElement) {
          statusElement.hidden = false;
          statusElement.classList.add('error');
          statusElement.dataset.i18n = 'viewer.invalid';
          statusElement.textContent = translations[currentLanguage()]['viewer.invalid'];
        }
        if (downloadLink) downloadLink.hidden = true;
        return;
      }

      const pdfPath = `assets/certificates/${encodeURIComponent(file)}`;
      if (downloadLink) {
        downloadLink.href = pdfPath;
        downloadLink.setAttribute('download', file);
        downloadLink.hidden = false;
      }
      if (originalLink) originalLink.href = pdfPath;
      ...
      try {
        ...
        loadedPdf = await window.pdfjsLib.getDocument(pdfPath).promise;
    ...
    window.addEventListener('resize', function () {
      ...
      resizeTimer = setTimeout(function () {
        renderPdfPages().catch(showViewerError);
      }, 250);
    });

    initializeCredentialViewer();
  }
